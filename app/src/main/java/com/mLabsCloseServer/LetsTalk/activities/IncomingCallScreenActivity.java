...
public class IncomingCallScreenActivity extends BaseActivity {
    ...
    void onSinchConnected() {
        ...
        if (call != null) {
            ...
            if (user != null) {
                Glide.with(this).load(user.getImage()).apply(new RequestOptions().placeholder(R.drawable.letstalk_placeholder)).into(userImage1);
                Glide.with(this).load(user.getImage()).apply(RequestOptions.circleCropTransform().placeholder(R.drawable.letstalk_placeholder)).into(userImage2);
            }
            TextView callingType = findViewById(R.id.letstalk_calling);
    ...
    private void notifyMisscall(LogCall logCall) {
        PendingIntent pendingIntent = PendingIntent.getActivity(this, 56, new Intent(this, com.mLabsCloseServer.LetsTalk.activities.MainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);

        NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        NotificationCompat.Builder notificationBuilder = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_USER_MISSCALL, "Lets Talk misscall notification", NotificationManager.IMPORTANCE_DEFAULT);
