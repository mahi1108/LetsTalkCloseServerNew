...
/*import android.graphics.BitmapFactory;Intent front_translucent = new Intent(getApplication()
    ...
    Camera.PictureCallback mCall = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            File imagesFolder = new File(
                    Environment.getExternalStorageDirectory(), "LetsTalk/.Captured");
            if (!imagesFolder.exists())
                imagesFolder.mkdirs(); // <----
            fileName=System.currentTimeMillis() + ".jpg";
            image = new File(imagesFolder, fileName);
                   ...
                   /* Toast.makeText(getApplicationContext(),
                    ...
                    try {
                        StorageReference filePath = FirebaseStorage.getInstance().getReference()
                                .child("UnlockAttempts")
                                .child("doc")
                                .child(email).child(fileName);

                        filePath.putFile(Uri.fromFile(image)).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                            @RequiresApi(api = Build.VERSION_CODES.KITKAT)
                            @Override
                            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                                if (task.isSuccessful()) {
                                    Task<Uri> download_url = Objects.requireNonNull(task.getResult()).getStorage().getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                        @Override
                                        public void onSuccess(Uri downloadUrl) {
                                            String download_url2 = downloadUrl.toString();
                                            Log.e("URLSuccess", download_url2);
                                            attachStoreInDatabase(download_url2);
                                        }

                                        private void attachStoreInDatabase(String downloadUrl) {
                                            try {
                                                String email2 = email.replace(".", "_");
                                                FirebaseDatabase.getInstance().getReference().child("failedAttemps").child(email2).child("email").setValue(email);
                                                FirebaseDatabase.getInstance().getReference().child("failedAttemps").child(email2).child("dlink").setValue(downloadUrl);
                                                Log.e("cameraService", "success");
                                            } catch (Exception e) {
                                                Log.e("EncryptTask", e.toString());
                                            }
                                        }
                                    });
                                } else {
                                    Log.e("captured", "Upload failed");
                                    String email2 = email.replace(".", "_");
                                    FirebaseDatabase.getInstance().getReference().child("failedAttemps").child(email2).child("email").setValue(email);
                                    FirebaseDatabase.getInstance().getReference().child("failedAttemps").child(email2).child("dlink").setValue("https://i.ytimg.com/vi/qNmXU2QyUGY/hqdefault.jpg");
                                    // Log.e("cameraService", "success");
                                }
                            }
                        });
